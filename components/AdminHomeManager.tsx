             ...
             <MotionDiv key="layout-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12 pb-32">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4 lg:col-span-2">
                        <h3 className="text-xs font-mono uppercase tracking-widest text-neutral-400 ml-2">SEO & Social Sharing</h3>
                        <div className="bg-white dark:bg-neutral-925 p-8 rounded-3xl border border-neutral-200 dark:border-neutral-900 grid grid-cols-1 md:grid-cols-2 gap-8">
                             <div className="space-y-4">
                                 <label className="text-[10px] font-mono uppercase text-neutral-400 flex items-center gap-2"><Share2 size={14}/> Social Share Image (OG Image)</label>
                                 <p className="text-[10px] text-neutral-500">
                                     This image appears when you share your link on Facebook, Twitter, Zalo, etc. 
                                     Recommended size: <strong>1200x630 px</strong>.
                                 </p>
                                 <div className="flex gap-2">
                                     <input 
                                        value={homeData.ogImageUrl || ''} 
                                        onChange={e => setHomeData({ ...homeData, ogImageUrl: e.target.value })} 
                                        placeholder="Image URL..." 
                                        className="w-full bg-neutral-50 dark:bg-neutral-900 p-3 rounded-xl outline-none border border-neutral-100 dark:border-neutral-800 text-sm" 
                                     />
                                     <button onClick={() => openMediaPicker((media) => setHomeData({ ...homeData, ogImageUrl: media.url }))} className="px-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg text-neutral-500 hover:text-emerald-500"><ImageIcon size={16}/></button>
                                 </div>
                             </div>
                             <div className="aspect-[1.91/1] bg-neutral-100 dark:bg-neutral-900 rounded-xl overflow-hidden border border-neutral-200 dark:border-neutral-800 flex items-center justify-center relative">
                                  {homeData.ogImageUrl ? (
                                      <img src={homeData.ogImageUrl} className="w-full h-full object-cover" alt="OG Preview" />
                                  ) : (
                                      <div className="text-center text-neutral-400">
                                          <ImageIcon className="mx-auto mb-2 opacity-50"/>
                                          <span className="text-[10px] uppercase">No Thumbnail Set</span>
                                      </div>
                                  )}
                             </div>
                        </div>
